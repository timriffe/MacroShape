...
It makes sense to plot a Lexis field if data contain a relationship that can be summarized with a line, a simple curve, or similar, that varies by age and/or time. All demographic forces (health, fertility, mortality, migration, and others) vary by age and time, and so any two variables produce a relationship that varies over age and time. For example, health behaviors and mortality outcomes might be thought to have a relationship that changes over age and time. Constructing a Lexis field involves several design choices, which can be codified into four basic steps. These steps to do so are outlined in the following description, referenced to regions of Fig. \@ref(fig:one).
