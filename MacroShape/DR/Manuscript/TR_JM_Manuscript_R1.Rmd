...
TODO TR: add area examples. Annotate Fig 1. Swap contour in Fig 4. Swap Pearson r for r2.
# Introduction
...
We give an overview of constructing a Lexis field with an application. Our example explores the relationship between remaining life expectancy and the standard deviation of remaining lifespan over age and time based on all available populations in the @HMD from 1950 onward. Variations in [COMMENT JMA: THIS SOUNDS AS JARGON, COULD WE REPRHASE THE FIRST PART OF THIS SENTENCE?] aesthetics of the field over age and time reveal macro-patterns between demographic variables. This example and others discussed motivate how Lexis fields could be used in other areas demographic research.
...
(ref:one)  A diagram depicting the translation of functional relationships in data conditioned on age and time to visual encoding on a Lexis field. **A**: Condition data selection on age and time. **B**: [COMMENT JMA: MAYBE JUST: 'FIT A MODEL, E.G. LINEAR, TO THE DATA SUBSET'] model the functional relationship in data subset. **C**: Translate the model to field elements, `pointers', using angle, and possibly also length, curvature, thickness, etc to encode model qualities. **D**: Populate the Lexis plane with field pointers to create a Lexis field.
