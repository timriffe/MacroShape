...
{\centering \includegraphics[width=0.8\linewidth]{Figures/Fig4} 

}

\caption{A Lexis map: Lexis surface of mean survivorship quantiles as a filled contour plot plus a Lexis field of \(e(x)\) by \(sd(x)\) linear fits, slopes exaggerated by 2. Pointer length is proportional to the diagonal of the IQR box around \(e(x)\) by \(sd(x)\), while grayscale and segment width are proportional to \(r^2\).}\label{fig:four}
\end{figure}
\FloatBarrier
