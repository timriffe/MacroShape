...
Constructing a Lexis field involves several desgrees of designer freedom, which can however be codified into four basic steps, which we summarize in Figure~\ref{fig:explain}. It only makes sense to plot a Lexis field if data contain a relationship that can be summarized with a line, curve, or similar, and that varies by age and/or time. A structuralThe first step is to determine the Lexis reference grid size for data selection, for example ___. Second, abstract a
